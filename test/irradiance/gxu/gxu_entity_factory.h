...
namespace gxu
{
	...
	template <typename factory> inline std::shared_ptr<gx::entity> create_lat_lon_sphere_entity( ID3D11Device* device, gx::shader_database* context, gx::indexed_draw_call_2 draw_call, math::float4 color )
	{
		return std::make_shared< gxu::static_mesh_entity<gx::indexed_draw_call_2, factory::type> > ( draw_call , factory::create( context, color) );
	}

    template <typename factory> inline std::shared_ptr<gx::entity> create_lat_lon_sphere_entity( gx::shader_database* context, gx::indexed_draw_call_2 draw_call, math::float4 kd, math::float4 ks_gloss )
	{
