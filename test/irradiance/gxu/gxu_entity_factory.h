...
namespace gxu
{
	...
	template <typename factory> inline std::shared_ptr<gx::entity> create_lat_lon_sphere_entity( ID3D11Device* device, gx::shader_database* context, gx::indexed_draw_call<2, gx::bit_16> draw_call, math::float4 color )
	{
    ...
    template <typename factory> inline std::shared_ptr<gx::entity> create_lat_lon_sphere_entity( gx::shader_database* context, gx::indexed_draw_call<2, gx::bit_16> draw_call, math::float4 kd, math::float4 ks_gloss )
	{
	...
	template <typename factory> inline std::shared_ptr<gx::entity> create_lat_lon_sphere_entity( ID3D11Device* device, gx::shader_database* context, float radius, uint32_t subdivision_count, math::float4 color )
    {
		return create_lat_lon_sphere_entity<factory> ( context, create_lat_lon_sphere( device, radius, subdivision_count) , color );
    }

    template <typename factory> inline std::shared_ptr<gx::entity> create_lat_lon_sphere_entity( ID3D11Device* device, gx::shader_database* context, float radius, uint32_t subdivision_count, math::float4 kd, math::float4 ks_gloss )
    {
