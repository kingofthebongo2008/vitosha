...
namespace math
{
	...
	inline float4 load1(const float* const __restrict address)
	{
		return _mm_load_ss(address);
	}

	inline float4 load2(const float* const address)
	{
		__m128i v = _mm_loadl_epi64( (const __m128i * ) address );
		return reinterpret_cast<__m128 *>(&v)[0];
	}

	inline float4 load3(const float* __restrict const address)
	{
